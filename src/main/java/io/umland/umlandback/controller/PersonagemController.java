...
class PersonagemController {
    ...
    @GetMapping("/{id}") public ResponseEntity<Personagem> buscar(@PathVariable Long id) { return repository.findById(id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build()); }
    @PostMapping
    public Personagem criar(@RequestBody Personagem p) { return repository.save(p); }
    @PutMapping("/{id}") public ResponseEntity<Personagem> atualizar(@PathVariable Long id, @RequestBody Personagem p) {
        return repository.findById(id).map(per -> {
            per.setNome(p.getNome()); per.setFala(p.getFala()); per.setImagem(p.getImagem());
