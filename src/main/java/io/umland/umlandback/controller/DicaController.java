...
class DicaController {
    ...
    @GetMapping("/{id}") public ResponseEntity<Dica> buscar(@PathVariable Long id) { return repository.findById(id).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build()); }
    @PostMapping
    public Dica criar(@RequestBody Dica d) { return repository.save(d); }
    @PutMapping("/{id}") public ResponseEntity<Dica> atualizar(@PathVariable Long id, @RequestBody Dica d) {
